...
#
# toggles between using a `hamiltnian` or a `quantum_LinearOperator` object
on_the_fly = False
# chain length
L = 20
# on-site spin size
...
for j, q in enumerate(qs):
    ...
    def f(i): return np.exp(-2j*np.pi*q*i/L)/np.sqrt(L)
    ...
    def f(i): return np.exp(-2j*np.pi*q*i/L)*np.sqrt(1.0/(2*L))
